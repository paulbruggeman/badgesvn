...
// these two variables are used for power management in main.c
unsigned int G_touchstamp=0;
unsigned int G_currentstamp=0;
...
/** timestamp of when a button last changed state, see touchCTMU.h
    (#LEFT_SLIDER) for array index constants. */
unsigned int buttonTimestamp[9] = {0};
unsigned int lastHandledTimestamp[9] = {0}; /**< timestamp of when a button was last handled */
...
//                case 3:
		...
		if ((buttonTimestamp[LEFT_SLIDER] == 0) && !(sampleButtonStatus & LEFT_SLIDER_MASK)) {
			buttonTimestamp[LEFT_SLIDER] = G_touchstamp = timestamp; /* apps can tell if start/stop by checking timestamp */
		...
		if ((buttonTimestamp[RIGHT_SLIDER] == 0) && !(sampleButtonStatus & RIGHT_SLIDER_MASK)) {
			buttonTimestamp[RIGHT_SLIDER] = G_touchstamp = timestamp; /* apps can tell if start/stop by checking timestamp */
		...
		if ((buttonTimestamp[TOP_SLIDER] == 0) && !(sampleButtonStatus & TOP_SLIDER_MASK)) {
			buttonTimestamp[TOP_SLIDER] = G_touchstamp = timestamp; /* apps can tell if start/stop by checking timestamp */
		...
		if ((buttonTimestamp[BOTTOM_SLIDER] == 0) && !(sampleButtonStatus & BOTTOM_SLIDER_MASK)) {
			buttonTimestamp[BOTTOM_SLIDER] = G_touchstamp = timestamp; /* apps can tell if start/stop by checking timestamp */
		...
		if ((buttonTimestamp[BUTTON] == 0) && !(sampleButtonStatus & BUTTON_MASK)) {
			buttonTimestamp[BUTTON] = G_touchstamp = timestamp; /* apps can tell if start/stop by checking timestamp */
