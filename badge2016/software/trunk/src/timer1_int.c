...
/*
    38khz IR timer code and interrupt code
    120 fps timer interrupt code
    LED PWM
    backlight PWM
...
// Timer Tick: 0.1 seconds
// Timer Prescale: 256
// Period = Freq/prescale * Timer Tick = (40000000/256)*0.1 = 15625
#define T5_Period   15625
...
void timerInit(void)
{
    ...
    OpenTimer5(T5_ON | T5_SOURCE_INT | T5_PS_1_256, T5_Period);
...
/* 
  ...
  almost certainly doesn't work for RC5 with a 38khz receiver
	...
	if (G_bitCnt > 31) {
		...
		if (G_IRerror) {
		   ...
		   G_IRerror = 0;
                ...
                else{ // track any ir packets we see
                    packets_seen = 1; // for sleep mode to un-toggle after wake
                    num_packets_seen++; // for main menu or apps to seen number and decrement
                }

		/* if packet is not for us don't keep it */
                /* MSS: If 448 or above, treat as bcast - allows for 6 bits of data in badgeId*/
...
void __ISR(_TIMER_5_VECTOR, IPL2SOFT) isr_stopwatch(){
    process_100ms_stopwatch();
    mT5ClearIntFlag();
}
