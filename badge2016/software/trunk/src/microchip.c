...
/* external non microchip main */
void ProcessIO(void);
void UserInit(void);

/**
 * Initialize system, check for USB device, and call ProcessIO() in endless
 * loop.
 * 
...
{
    return CDC_DATA_IN_EP_SIZE;
}

unsigned int USBbufferSizeOut()
{
    return CDC_DATA_OUT_EP_SIZE;
}
...
/**
 * True if USB not yet configured, or suspended.
 * @return 0 if USB is ready
 */
volatile int mchipUSBnotReady()
{
    return ((USBDeviceState < CONFIGURED_STATE)||(USBSuspendControl==1));
}

volatile unsigned char getUSBSuspendControl() 
{
    return USBSuspendControl;
}
...
/** ******************************************************************
 * A centralized initialization routine. All required USB 
 * initialization routines are called from here.
 *
 * User application initialization routine should also be called
 * from here.                  
 *******************************************************************/
static void InitializeSystem(void)
{
