...
// Set this before enqueing a send packet to have the IR handler wait
// before sending the next packet (it may not actually be your packet thoguh :P)
...
void IRhandler() 
{
    ...
    /* curr == next == empty */
    if ((IRpacketInCurr != IRpacketInNext) && !send_delay) {
...
void ir_ping(struct IRpacket_t p)
{
    ...
    if(p.data & PING_REQUEST)
    {
        ...
        pkt.p.data    = PING_RESPONSE | (0x1FF & G_sysData.badgeId);
...
//-----------------------CHANGE THIS----------------
extern unsigned char draw_ir_image[16][32];
extern unsigned char draw_image_received;
unsigned char ir_j = 0;
unsigned char ir_k = 0;

void ir_draw_app(struct IRpacket_t p){
    
    static unsigned char first_rcv_row = 0; 
    first_rcv_row |= (p.badgeId & 0b000011111);
    if(ir_j == 0 && ir_k == 0){
        ir_k = first_rcv_row;
    }
    if(ir_j < 14){
        
        draw_ir_image[ir_j][ir_k] |= (p.data & 0b0000000011111111);
        ir_j++;
        draw_ir_image[ir_j][ir_k] |= p.data >> 8;
        ir_j++;
    }
    else{
        ir_j= 0;
    ...
    if(ir_j == 15 && ir_k == 31){
        ir_j = 0;
        ir_k = 0;
        draw_image_received = 1;
    }
    if((p.badgeId & 0b000100000) == 0b000100000){
        ir_j = 0;
        ir_k = 0;
        draw_image_received = 1;
    }
...
{
    //
    // Force them to put in someones Id and pair up
    if(p.badgeId == 0 || p.badgeId > 400)
        ctf_print_ser("some's bcasting for flags...");
    else
        ctf_print_ser("FLAG:IFIR3C4LL");
    //DEBUGSTRLVL(DNOISE, "ir_app5");

}
//Not needed anymore used by ir_screensaver
...
{
    //unsigned char code = p.data >> 12;
    //blue(100);
    if(p.data == ADMIN_SILENCE){
        G_mute = 1;
        red(100);
    }
    else if(p.data == ADMIN_GOCRAY){
        
    }
    else if(p.data == ADMIN_ANGRY){
