...
#define MSG_PACKET_DELTA 7000 // lower this number to send packets faster
...
struct menu_t messenger_m[] = {
    ...
    {"TTL:", VERT_ITEM, FUNCTION, {(struct menu_t *)msg_set_relay}},
    {"Exit", VERT_ITEM|LAST_ITEM, FUNCTION, {(struct menu_t *) msg_set_exit}},
};


// Only have 4 bits for relay
unsigned char max_configurable_relay_amount = 15;
...
void msg_populate_menu()
{
    ...
    messenger_m[MSG_RELAY_M_IDX].name[4] = '0' + (relay_amount/100) % 10;
    messenger_m[MSG_RELAY_M_IDX].name[5] = '0' + (relay_amount/10) % 10;
    messenger_m[MSG_RELAY_M_IDX].name[6] = '0' + (relay_amount) % 10;
    messenger_m[MSG_RELAY_M_IDX].name[8] = 0;
}
