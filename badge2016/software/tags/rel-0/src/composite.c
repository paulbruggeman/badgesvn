...
void DlInit() {
    ...
    G_dl.pos.x = 0;
    G_dl.pos.y = 0;
    G_dl.sl = 0; /* next scanline to render */
...
{
    G_dl.displayList[G_dl.currItem].type = LCD_TRANSPARENCY;
    G_dl.displayList[G_dl.currItem].dData.transparencyMask = transparencyMask;
    DLNEXT;
}
...
//    static unsigned char sl=0;
	...
	for (item=0; item < G_dl.currItem; item++) {
	    DEBUG(printf("\n-- sl %d item %d type %s\n", G_dl.sl, item, LCD_Functions[G_dl.displayList[item].type]);)
	    DEBUG(fflush(stdout);)
		...
		case LCD_TRANSPARENCY: /* 8 bit transparency */
		    TRMask = G_dl.displayList[item].dData.transparencyMask;
		    G_dl.stats.transparency++;
		    break;
		...
		case LCD_SPAN:  /* (x2 8bit) fill w color from last x->x2 span  */
			...
			if (ep >= LCD_XSIZE) ep = (LCD_XSIZE-1);
			    ...
			    if (p < LCD_XSIZE) {
			            if (TRMask > 0) 
					scanLine[p] = (scanLine[p] & (~TRMask)) | (FGColor & TRMask);
				    else
			        	scanLine[p] = FGColor;
			    }
			}

	    	        DEBUG(printf("G_dl.sl %d LCD_SPAN px %d dl.x %d py %d\n", G_dl.sl, pos.x, G_dl.displayList[item].dData.pos.x, pos.y);)
	    	        DEBUG(fflush(stdout);)
		...
		case LCD_FONT_SPAN:
			...
			for (p = 0; p < assetList[assetId].x ; p++) {
				...
				if (ci == 0) {
			            if (TRMask > 0) 
					scanLine[p + pos.x] = (scanLine[p + pos.x] & (~TRMask)) | (BGColor & TRMask);
				    else
					scanLine[p + pos.x] = BGColor;
				}
				else {
			            if (TRMask > 0) 
					scanLine[p + pos.x] = (scanLine[p + pos.x] & (~TRMask)) | (FGColor & TRMask);
				    else
			                scanLine[p + pos.x] = FGColor;
				}
			    }
			}
	    		DEBUG(printf("G_dl.sl %d FONT_SPAN assetId %d posx %d posy %d aux scanline %d\n", 
				G_dl.sl, assetId, pos.x, pos.y, G_dl.displayList[item].aux.scanLine);)
			...
			case PICTURE8BIT:
			    ...
			    for (p = 0; p < assetList[assetId].x; p++) {
			        ...
			        if (ci != TRIndex) { /* transparent? */
			            ...
			            /* pos.x == offset into scan buffer */
			            if (TRMask > 0)
					scanLine[p + pos.x] = (scanLine[p + pos.x] & (~TRMask)) | (pixel & TRMask);
			            else
...
//						+ G_dl.displayList[item].dData.pic.imageNo * (assetList[assetId].x >> 3) * assetList[assetId].y]));
				...
				for (p=0; p < (assetList[assetId].x); p += 8) {
				    unsigned char bit;

				    pixbyte = *pixdata++; 

				    for (bit=0; bit < 8; bit++) { /* 8 pixels per byte */
				        if ((bit + pos.x) > (LCD_XSIZE-1)) continue; /* clip x */

				        ci = ((pixbyte >> bit) & 0x1); /* ci = color index */
				        if (ci != TRIndex) { /* transparent? */
					    /* pos.x == offset into scan buffer */
					    if (ci == 0) {
					        //scanLine[p + pos.x + bit] = BGColor;
				                if (TRMask > 0) 
				                    //scanLine[p + pos.x + bit] |= (BGColor & TRMask);
						    scanLine[p + pos.x + bit] = (scanLine[p + pos.x + bit] & (~TRMask)) | (BGColor & TRMask);
					        else
				                    scanLine[p + pos.x + bit] = BGColor;
					    } else {
					        //scanLine[p + pos.x + bit] = FGColor;
				                if (TRMask > 0) 
				                    //scanLine[p + pos.x + bit] |= (FGColor & TRMask);
						    scanLine[p + pos.x + bit] = (scanLine[p + pos.x + bit] & (~TRMask)) | (FGColor & TRMask);
					        else
				                    scanLine[p + pos.x + bit] = FGColor;
			...
			default:
	    		...
	    		DEBUG(printf("G_dl.sl %d PICTURE_SPAN assetId %d posx %d posy %d aux scanline %d\n", 
				G_dl.sl, assetId, pos.x, pos.y, G_dl.displayList[item].aux.scanLine);)
	...
	DEBUG(printf("G_dl.sl %d LCD SEND assetId %d posx %d posy %d aux.scanline %d\n", G_dl.sl, assetId, pos.x, pos.y, G_dl.displayList[item].aux.scanLine);)
	DEBUG(fflush(stdout);)
