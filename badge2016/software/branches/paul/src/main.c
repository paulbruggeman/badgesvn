...
void UserInit(void)
{
    ...
    /* RGB LED */
    RED_TRIS = 0;    /* output */
    RED_PIN = 0;      /* red init low */
    RED_PULLDOWN = 0;    /* pulldown == off */


    BLUE_TRIS = 0;    /* output */
    BLUE_PIN = 0;      /* blue init low */
    BLUE_PULLDOWN = 0;    /* pulldown == off */

    GREEN_TRIS = 0;    /* output */
    GREEN_PIN = 0;      /* green init low */
    GREEN_PULLDOWN = 0;    /* pulldown == off */
    ...
    GREEN_PIN = 1;      /* BLUE */
    BACKLIGHT_PIN = 1;      /* backlight on. you will see nothing if it is off */
       
    //unused (was Ghost Game Pin (left most programming pin))
    //LATAbits.LATA7 = 0;
    
    /* button init */
    BUTTON_TRIS = 1; // button == input
    BUTTON_PULLUP = 1; // pullup == on
    ...
    SPEAKER_TRIS = 0;	// piezo == output
    SPEAKER_PIN = 0;      // piezo init off
    SPEAKER_PULLUP = 0;    // RA9  pull up == off
    SPEAKER_PULLDOWN = 0;    /* pulldown == off */
...
extern unsigned int currentstamp;
extern unsigned int touchstamp;
// dormant returns 1 if dormant for 2 minutes, otherwise returns 0
int dormant(void) {
		unsigned char tmpstr[32];
		sprintf(tmpstr, "Dormant: %u\r\n", currentstamp);
		DEBUGSTR(tmpstr);
    if (currentstamp >= (touchstamp + 860000)){
        if (IRpacketInCurr == IRpacketInNext) {  
            return 1;
        }
        else {
            // wake on IR receive and reset timer
            touchstamp = currentstamp;
            return 0;
        }
    } 
    else {
...
#define mInitAllLEDs()      PORTB_PINS &= 0x0; PORTB_PINS &= 0x0;

#define mLED_1              RED_PIN /* red */
#define mLED_2              BLUE_PIN  /* blue */
#define mLED_3              GREEN_PIN /* green */
