...
void timerInit(void)
{
    ...
    INT1ENABLE=0; // 2015 disable this interrupt
/*
...
*/
    ...
    IR_REC_TRIS = 1; // 2015 IR IN
    IR_REC_PULLDOWN = 0;  // pulldown off
    IR_REC_PULLUP = 0;  // pullup off
...
*/
    ...
    IRLED_TRIS = 0; // 2015 IR == OUTPUT
    IRLED_PULLDOWN = 0;  // pulldown off
    IRLED_PULLUP = 1;  // pullup on == IR off since driving transistor inverts val
...
*/
    CFGIOLOCK = 0; // unlock configuration
/*
    INT1Rbits.INT1R = 0b0010; // remap RPB0 to INT1
*/
    INT1REMAP = 0b0010; // remap RPB0 to INT1
    CFGIOLOCK = 1; // relock configuration
/*
...
void __ISR(_TIMER_2_VECTOR, IPL2SOFT) Timer2Handler(void)
{
   ...
   if (G_IRrecv == 1) {
	if (G_bitCnt > 31) {
	    ...
	    if (G_halfCount == 16) G_firstHalf = !IR_REC_PIN; 
	    if (G_halfCount == 48) G_lastHalf = !IR_REC_PIN;
