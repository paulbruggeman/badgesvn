...
const struct menu_t extra_m[] = {
    ...
    {"Back", VERT_ITEM|DEFAULT_ITEM|LAST_ITEM, BACK, {NULL}},
};
...
void bowl_cb()
{
	...
	switch (state) {
		...
		case BOWL_DRAW_BG:
			...
			DlColor(RED);
			DlLine(64, 48+cursor.y-16, 64, 80+cursor.y-16);
			DlLine(48, 64+cursor.y-16, 80, 64+cursor.y-16);

			DlLine(48, 48+cursor.y-16, 80, 80+cursor.y-16);
			DlLine(48, 80+cursor.y-16, 80, 48+cursor.y-16);
		...
		case BOWL_SLEEPLOOP:
			if (cnt == 100) { /* kill time so ball doesnt move too fast */
				state++;
